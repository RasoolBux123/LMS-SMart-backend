...
def _submission_row(
    doc: dict, id_field: str, *, for_student: bool = False
) -> dict:
    score = doc.get("score")
    if score is None:
...
def create_coursework_router(
    ...
    async def get_item(
        ...
        if user["role"] == "student":
            ...
            published = status_val == "published" or (
                status_val is None and doc.get("isPublished", True)
            )
            if not published:
    ...
    async def update_item(
        ...
        was_published = existing.get("status") == "published" or (
            existing.get("status") is None and existing.get("isPublished", False)
        )
        ...
        if payload.status == "published" and not was_published:
            try:
                await notify_enrolled_students(
                    course_id=str(
                        payload.courseId or existing.get("courseId") or ""
                    ),
    ...
    async def submit_work(
        ...
        if not published:
            raise HTTPException(
                status_code=403,
                detail="This item is not open for submissions",
            )
        ...
        max_mb = int(
            item.get("maxFileSizeMb") or item.get("max_file_size_mb") or 25
        )
        ...
        return {
            "data": _submission_row(doc, id_field),
            "message": "submitted",
        }

    # ── Student: my submission (FIXED) ──────────────────────────────
    ...
    async def my_submission(
        ...
        docs = await (
            db[sub_coll]
            .find({id_field: item_id, "studentId": str(user["_id"])})
    ...
    # ── Instructor: list submissions (FIXED) ────────────────────────
    ...
    async def list_submissions(
        ...
        cursor = db[sub_coll].find(
            {"$or": [{id_field: item_id}, {"assignmentId": item_id}]}
        )
        rows = []
        async for s in cursor:
            row = _submission_row(s, id_field)
            if (
                row.get("marksAwarded") is not None
    ...
    async def release_marks(
        ...
        result = await db[sub_coll].update_many(
            {id_field: item_id, "marksHidden": True},
...
async def grade_submission(
    ...
    score = (
        ...
        if payload.marksAwarded is not None
        else payload.score
    )
    if score is None:
    ...
    for sub_coll, id_field in (
        ...
        return {
            "data": _submission_row(updated, id_field),
            "message": "graded",
        }
