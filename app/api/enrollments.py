...
async def enroll_student(
    ...
    db = database.db
    if not ObjectId.is_valid(payload.courseId) or not ObjectId.is_valid(
        payload.userId
    ):
    ...
    if not await _instructor_can_access_course(
        db, user, course, payload.courseId
    ):
...
async def list_course_enrollments(
    ...
    cursor = db.enrollments.find({"courseId": course_id}).sort(
        "enrolledAt", -1
    )
    rows = []
